...
class Reconstructor:
    ...
    def __init__(self):
        # vars
        self.gladefile = '/usr/lib/Constructor/Constructor.glade'
        self.iconFile = '/usr/lib/Constructor/icon.png'

        self.appName = "Constructor2 SW"        
        ...
        # i18n for menu item
        menuName = _("Remastering Tool for Live")
        ...
        # set values
        if os.path.exists(os.environ['HOME'] + "/.Constructor/currentProject"):
            currentProject = commands.getoutput("cat ~/.Constructor/currentProject")
        else:
        ...
        if os.path.exists(os.path.join(self.customDir, "iso_name")):
            ...
            iso_name = "SonicOS <VERSION> <EDITION> <XX>-bit"
    ...
    def checkWorkingDir(self):
        ...
        remasterExists = None
        rootExists = None
        if os.path.exists(os.path.join(self.customDir, "ISO")) == False:
            if self.wTree.get_widget("radiobutton_new_project").get_active() == False:
                remasterExists = False
        if os.path.exists(os.path.join(self.customDir, "FileSystem")) == False:
            if self.wTree.get_widget("radiobutton_new_project").get_active() == False:
                rootExists = False
        workingDirOk = True
        ...
        if workingDirOk == False:
            ...
            lblRemasterText = _("  There is no FileSystem directory.  Please select create ISO option.  ")
            lblRootText = _("  There is no root directory.  Please select create FileSystem option.  ")
    ...
    def checkPage(self, pageNum):
        if pageNum == self.pageLiveSetup:
            ...
            warnDlg.vbox.pack_start(labelSpc)
            labelSpc.show()
            lblBuildText = _("  <b>Build Live Image?</b>  ")
    ...
    def checkEnableBurnIso(self):
        ...
        if commands.getoutput('which brasero') != '':
    ...
    def launchTerminal(self):
        try:
            # setup environment
            os.popen('xhost +local:')
            # copy Hosts info
            print _("Copying Hosts info...")
            os.popen('cp -f /etc/hosts ' + os.path.join(self.customDir, "FileSystem/etc/hosts"))
            # copy dns info
            print _("Copying DNS info...")
            os.popen('cp -f /etc/resolv.conf ' + os.path.join(self.customDir, "FileSystem/etc/resolv.conf"))
            # mount /dev
            print _("Mounting /dev filesystem...")
            os.popen('mount --bind /dev \"' + os.path.join(self.customDir, "FileSystem/dev") + '\"')
            # mount /proc
            print _("Mounting /proc filesystem...")
            os.popen('mount --bind /proc \"' + os.path.join(self.customDir, "FileSystem/proc") + '\"')
            # mount /sys
            print _("Mounting /sys filesystem...")
            os.popen('mount --bind /sys \"' + os.path.join(self.customDir, "FileSystem/sys") + '\"')
            # mount /dev/pts
            print _("Mounting /dev/pts filesystem...")
            os.popen('mount --bind /dev/pts \"' + os.path.join(self.customDir, "FileSystem/dev/pts") + '\"')
            # copy apt.conf
            #print _("Copying apt.conf configuration...")
            #os.popen('cp -f /etc/apt/apt.conf ' + os.path.join(self.customDir, "FileSystem/etc/apt/apt.conf"))
            # copy wgetrc
            print _("Copying wgetrc configuration...")
            # backup
            os.popen('mv -f \"' + os.path.join(self.customDir, "FileSystem/etc/wgetrc") + '\" \"' + os.path.join(self.customDir, "FileSystem/etc/wgetrc.orig") + '\"')
            os.popen('cp -f /etc/wgetrc ' + os.path.join(self.customDir, "FileSystem/etc/wgetrc"))
            # HACK: create temporary script for chrooting
            scr = '#!/bin/sh\n#\n#\t(c) reconstructor, 2006\n#\nchroot ' + os.path.join(self.customDir, "FileSystem/") + '\n'
            ...
            if commands.getoutput('which x-terminal-emulator') != '':
                ...
                if os.path.exists("/usr/bin/xterm"):
                    os.popen('export HOME=/root ; xterm -bg black -fg blue -rightbar -title \"%s\" -e /tmp/reconstructor-terminal.sh' % self.folder)
                else:
            ...
            os.popen('mv -f \"' + os.path.join(self.customDir, "FileSystem/etc/wgetrc.orig") + '\" \"' + os.path.join(self.customDir, "FileSystem/etc/wgetrc") + '\"')
            # remove apt.conf
            #print _("Removing apt.conf configuration...")
            #os.popen('rm -Rf \"' + os.path.join(self.customDir, "FileSystem/etc/apt/apt.conf") + '\"')
            # remove Hosts
            print _("Removing hosts info...")
            os.popen('rm -f \"' + os.path.join(self.customDir, "FileSystem/etc/hosts") + '\"')
            # umount /dev/pts
            print _("Umounting /dev/pts...")
            os.popen('umount \"' + os.path.join(self.customDir, "FileSystem/dev/pts/") + '\"')
            # umount /sys
            print _("Umounting /sys...")
            os.popen('umount \"' + os.path.join(self.customDir, "FileSystem/sys/") + '\"')
            # umount /proc
            print _("Umounting /proc...")
            os.popen('umount \"' + os.path.join(self.customDir, "FileSystem/proc/") + '\"')
            # umount /dev
            print _("Umounting /dev...")
            os.popen('umount \"' + os.path.join(self.customDir, "FileSystem/dev/") + '\"')
            # remove temp script
    ...
    def burnIso(self):
        try:
            if commands.getoutput('which brasero') != '':
                print _('Burning ISO...')
                os.popen('brasero --source-iso=\"' + self.buildLiveCdFilename + '\"')
            else:
                print _('Error: brasero is needed for burning iso files... ')
    ...
    def startInteractiveEdit(self):
        ...
        try:
            ...
            #os.popen('chroot \"' + os.path.join(self.customDir, "FileSystem/") + '\" ' + 'su -c \"export DISPLAY=localhost:1 ; gnome-session\" 1>&2 2>/dev/null\"')
    ...
    def saveSetupInfo(self):
        ...
        os.system("mkdir -p ~/.Constructor")
        os.system("echo \"" + self.customDir + "\" > ~/.Constructor/currentProject")
...
# ---------- Setup ---------- #
    def setupWorkingDirectory(self):
        ...
        if self.createNewProject:
            ...
            #if os.path.exists(os.path.join(self.customDir, "ISO")):
            #    print _("INFO: Removing existing ISO directory...")
            #    os.popen('rm -Rf \"' + os.path.join(self.customDir, "ISO/") + '\"')
            if os.path.exists(os.path.join(self.customDir, "ISO")) == False:
                print "INFO: Creating ISO directory..."
                os.makedirs(os.path.join(self.customDir, "ISO"))
            # check for iso
            if self.isoFilename == "":
            ...
            # copy remaster files
            os.popen('rsync -at --del ' + self.mountDir + '/ \"' + os.path.join(self.customDir, "ISO") + '\"')
        ...
        # custom root dir
        if self.createNewProject:
            #if os.path.exists(os.path.join(self.customDir, "FileSystem")):
            #    print _("INFO: Removing existing Custom FileSystem directory...")

            #    os.popen('rm -Rf \"' + os.path.join(self.customDir, "FileSystem/") + '\"')
            if os.path.exists(os.path.join(self.customDir, "FileSystem")) == False:
                print _("INFO: Creating Custom FileSystem directory...")
                os.makedirs(os.path.join(self.customDir, "FileSystem"))
            # check for existing directories and remove if necessary
            ...
            if self.isoFilename == "":
                ...
                response = mntDlg.run()
                if response == gtk.RESPONSE_OK:
                    print _("Using Live CD for squashfs FileSystem...")
                    mntDlg.destroy()
                    os.popen("mount " + self.mountDir)
                else:
                    mntDlg.destroy()
                ...
                print _("Using ISO for squashfs FileSystem...")
                os.popen('mount -o loop \"' + self.isoFilename + '\" ' + self.mountDir)
            ...
            print _("Extracting squashfs FileSystem...")

            # copy squashfs root
            os.popen('rsync -at --del \"' + os.path.join(self.customDir, "tmpsquash") + '\"/ \"' + os.path.join(self.customDir, "FileSystem/") + '\"')
            ...
            os.popen('chmod 6755 \"' + os.path.join(self.customDir, "FileSystem/usr/bin/sudo") + '\"')
            os.popen('chmod 0440 \"' + os.path.join(self.customDir, "FileSystem/etc/sudoers") + '\"')
            print _("Finished extracting squashfs FileSystem...")
...
# ---------- Build ---------- #
    def build(self):
        ...
        vmlinuz_filename = commands.getoutput("ls -al %s/FileSystem/vmlinuz" % self.customDir).split("/")[-1]
        vmlinuz_path = "%s/FileSystem/boot/%s" % (self.customDir, vmlinuz_filename)
        if os.path.exists(vmlinuz_path):                
            print "Updating vmlinuz"
            os.popen("cp %s %s/ISO/casper/vmlinuz" % (vmlinuz_path, self.customDir))            
        else:
            print "WARNING: Not updating vmlinuz!!! %s not found!" % vmlinuz_path
            return
        initrd_filename = commands.getoutput("ls -al %s/FileSystem/initrd.img" % self.customDir).split("/")[-1]
        initrd_path = "%s/FileSystem/boot/%s" % (self.customDir, initrd_filename)
        if os.path.exists(initrd_path):                         
            print "Updating initrd"            
            os.popen("mkdir -p %s/ISO/.disk" % self.customDir)
            os.popen("rm -rf %s/ISO/.disk/*uuid*" % self.customDir)
            tempdir = tempfile.mkdtemp()
            cwd = commands.getoutput("pwd")
            os.chdir(tempdir)
            if "gzip" in commands.getoutput("file %s" % initrd_path).lower():
                os.popen("cp %s %s/ISO/casper/initrd.gz" % (initrd_path, self.customDir))                        
                os.popen("gzip -cd \"%s/ISO/casper/initrd.gz\" -S \".gz\" | cpio -id" % self.customDir)                
                ...
                for link in links.split("\n"):                    
                    ...
                    if link_target[0] == "/":
                        ...
                        new_path = "%s/FileSystem/%s" % (self.customDir, new_link_target)                  
                ...
                os.popen("find . | cpio --quiet --dereference -o -H newc | gzip -9c > \"%s/ISO/casper/initrd.gz\"" % self.customDir)            
                os.popen("cp conf/uuid.conf %s/ISO/.disk/casper-uuid-generic" % self.customDir)
                os.popen("cp conf/uuid.conf %s/ISO/.disk/live-uuid-generic" % self.customDir)               
                os.popen("mv %s/ISO/casper/initrd.gz %s/ISO/casper/initrd.lz" % (self.customDir, self.customDir))                
            else:
                os.popen("cp %s %s/ISO/casper/initrd.lz" % (initrd_path, self.customDir))                
                os.popen("lzma -cd \"%s/ISO/casper/initrd.lz\" -S \".lz\" | cpio -id" % self.customDir)
                links = commands.getoutput("find . -type l | (while read FN ; do ls -ld \"$FN\"; done)")
                for link in links.split("\n"):                    
                    link_target = link.split(" ")[-1]
                    link_name = link.split(" ")[-3]
                ...
                if broken_links != "0":  
                    ...
                    return
                os.popen("uuidgen -r > conf/uuid.conf")                
                os.popen("find . | cpio --quiet --dereference -o -H newc | lzma -9c > \"%s/ISO/casper/initrd.lz\"" % self.customDir)            
                os.popen("cp conf/uuid.conf %s/ISO/.disk/casper-uuid-generic" % self.customDir)
                os.popen("cp conf/uuid.conf %s/ISO/.disk/live-uuid-generic" % self.customDir)
            os.chdir(cwd)
        ...
        os.popen("du -b %(directory)s/FileSystem/ 2> /dev/null | tail -1 | awk {'print $1;'} > %(directory)s/ISO/casper/filesystem.size" % {'directory':self.customDir})
        ...
        if os.path.exists(os.path.join(self.customDir, "FileSystem")):
            print _("Creating SquashFS FileSystem...")
            print _("Updating File lists...")
            q = ' dpkg-query -W --showformat=\'${Package} ${Version}\n\' '
            os.popen('chroot \"' + os.path.join(self.customDir, "FileSystem/") + '\"' + q + ' > \"' + os.path.join(self.customDir, "ISO/casper/filesystem.manifest") + '\"' )
            os.popen('cp -f \"' + os.path.join(self.customDir, "ISO/casper/filesystem.manifest") + '\" \"' + os.path.join(self.customDir, "ISO/casper/filesystem.manifest-desktop") + '\"')
            # check for existing squashfs FileSystem
            if os.path.exists(os.path.join(self.customDir, "ISO/casper/filesystem.squashfs")):
                print _("Removing existing SquashFS FileSystem...")
                os.popen('rm -Rf \"' + os.path.join(self.customDir, "ISO/casper/filesystem.squashfs") + '\"')
            print _("Building SquashFS FileSystem...")
            # check for alternate mksquashfs
            if mksquashfs == '':
                os.system('mksquashfs \"' + os.path.join(self.customDir, "FileSystem/") + '\"' + ' \"' + os.path.join(self.customDir, "ISO/casper/filesystem.squashfs") + '\" -comp gzip')
            else:
                os.system(mksquashfs + ' \"' + os.path.join(self.customDir, "FileSystem/") + '\"' + ' \"' + os.path.join(self.customDir, "ISO/casper/filesystem.squashfs") + '\"')

        # build iso       
        if os.path.exists(os.path.join(self.customDir, "ISO")):
            print _("Creating ISO...")
            # update manifest files
            os.system("/usr/lib/Constructor/updateManifest.sh " + self.customDir)
            # update md5
            print _("Updating md5 sums...")
            os.system('rm ' + os.path.join(self.customDir, "ISO/") + ' md5sum.txt')
            os.popen('cd \"' + os.path.join(self.customDir, "ISO/") + '\"; ' + 'find . -type f -print0 | xargs -0 md5sum > md5sum.txt')
            #Remove md5sum.txt from md5sum.txt
            os.system("sed -e '/md5sum.txt/d' " + os.path.join(self.customDir, "ISO/") + "md5sum.txt > " + os.path.join(self.customDir, "ISO/") + "md5sum.new")
            os.system("mv " + os.path.join(self.customDir, "ISO/") + "md5sum.new " + os.path.join(self.customDir, "ISO/") + "md5sum.txt")
            #Remove boot.cat from md5sum.txt
            os.system("sed -e '/boot.cat/d' " + os.path.join(self.customDir, "ISO/") + "md5sum.txt > " + os.path.join(self.customDir, "ISO/") + "md5sum.new")
            os.system("mv " + os.path.join(self.customDir, "ISO/") + "md5sum.new " + os.path.join(self.customDir, "ISO/") + "md5sum.txt")
            #Remove isolinux.bin from md5sum.txt
            os.system("sed -e '/isolinux.bin/d' " + os.path.join(self.customDir, "ISO/") + "md5sum.txt > " + os.path.join(self.customDir, "ISO/") + "md5sum.new")
            os.system("mv " + os.path.join(self.customDir, "ISO/") + "md5sum.new " + os.path.join(self.customDir, "ISO/") + "md5sum.txt")
            # remove existing iso
            ...
            os.system('genisoimage -o \"' + self.buildLiveCdFilename + '\" -b \"isolinux/isolinux.bin\" -c \"isolinux/boot.cat\" -no-emul-boot -boot-load-size 4 -boot-info-table -V \"' + self.LiveCdDescription + '\" -cache-inodes -r -J -l \"' + os.path.join(self.customDir, "ISO") + '\"')                
...
if __name__ == "__main__":
    ...
    if os.getuid() != 0 :
        ...
        warnDlg = gtk.Dialog(title="Constructor2 SW", parent=None, flags=0, buttons=(gtk.STOCK_OK, gtk.RESPONSE_OK))
        warnDlg.set_icon_from_file('/usr/lib/Constructor/icon.png')
        warnDlg.vbox.set_spacing(10)
        labelSpc = gtk.Label(" ")
        warnDlg.vbox.pack_start(labelSpc)
        labelSpc.show()
